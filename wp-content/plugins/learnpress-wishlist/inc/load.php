	...
	class LP_Addon_Wishlist extends LP_Addon {
		...
		public function toggle_course_wishlist() {
			...
			if ( ! wp_verify_nonce( $nonce, 'course-toggle-wishlist' ) ) {
				die ( __( 'Không đủ quyền để thực hiện tác vụ', 'learnpress-wishlist' ) );
			}
			...
			learn_press_send_json(
				array(
					...
					'message'     => '',
					'button_text' => $state != 'on' ? __( 'Thêm vào danh sách yêu thích',
						'learnpress-wishlist' ) : __( 'Xóa khỏi danh sách yêu thích', 'learnpress-wishlist' )
				)
			);
		}
		...
		private function _get_state_title( $state = 'on' ) {
			return $state == 'on' ? __( 'Xóa khóa học này khỏi danh sách yêu thích',
				'learnpress-wishlist' ) : __( 'Thêm khóa học này vào danh sách yêu thích', 'learnpress-wishlist' );
		}
		...
		private function _get_state_message( $state = 'on' ) {
			return $state == 'on' ? __( 'Khóa học này đã được thêm vào danh sách yêu thích của bạn',
				'learnpress-wishlist' ) : __( 'Khóa học này đã được xóa khỏi danh sách yêu thích', 'learnpress-wishlist' );
		}
		...
		public function wishlist_tab( $tabs ) {
			$tabs[ $this->get_tab_slug() ] = array(
				'title'    => __( 'Mục yêu thích', 'learnpress-wishlist' ),
