	...
	class LP_Addon_Course_Review extends LP_Addon {
		...
		public function review_assets() {
			...
			if ( learn_press_is_course() ) {
				...
				wp_localize_script( 'course-review', 'learn_press_course_review',
					array(
						'localize' => array(
							'empty_title'   => __( 'Vui lòng nhập tiêu đề', 'learnpress-course-review' ),
							'empty_content' => __( 'Vui lòn nhập nội dung', 'learnpress-course-review' ),
							'empty_rating'  => __( 'Vui lòng chọn đánh giá', 'learnpress-course-review' )
						)
					)
				);
			}
		...
		public function add_review() {
			...
			if ( wp_verify_nonce( $nonce, 'learn_press_course_review_' . $id ) ) {
				...
				$response['message'] = __( 'Lỗi', 'learnpress-course-review' );
			}

			if ( get_post_type( $id ) != 'lp_course' ) {
				$response['result']  = 'fail';
				$response['message'] = __( 'Khóa học không hợp lệ', 'learnpress-course-review' );
			}
		...
		public function add_comment_type_filter( $cmt_types ) {
			$cmt_types[ self::$comment_type ] = __( 'Đánh giá khóa học', 'learnpress-course-review' );
		...
		public function add_course_tab_reviews( $tabs ) {
			$tabs['reviews'] = array(
				'title'    => __( 'Đánh giá', 'learnpress-course-review' ),
				'priority' => 60,
