...
<div class="course-rate">
	...
	$text = sprintf( _n( '%d đánh giá', '%d đánh giá', $total, 'learnpress-course-review' ), $total );
	?>
    ...
    <div>
		...
		if ( isset( $course_rate_res['items'] ) && ! empty( $course_rate_res['items'] ) ):
			foreach ( $course_rate_res['items'] as $item ):
				?>
                <div class="course-rate">
                    <span><?php esc_html_e( $item['rated'] ); ?><?php _e( 'Sao', 'learnpress-course-review' ); ?></span>
                    <span><?php esc_html_e( $item['total'] ); ?><?php _e( 'Đánh giá', 'learnpress-course-review' ); ?></span>
