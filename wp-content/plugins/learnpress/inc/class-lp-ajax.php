		...
		public static function recover_order() {
			...
			if ( is_wp_error( $order ) ) {
				...
				$result['message']  = sprintf(
					__( 'Đơn hàng %s đã được khôi phục lại.', 'learnpress' ),
					$order_key
				);
		...
		public static function checkout_user_email_exists() {
			...
			if ( email_exists( $email ) ) {
				...
				$output             = '<div class="lp-guest-checkout-output">' . __(
					'Email này đã được đăng ký. Bạn có chắc muốn tiếp tục với email này chứ?',
					'learnpress'
				) . '</div>';
			} else {
		...
		public static function complete_lesson() {
			...
			try {
				...
				if ( ! is_wp_error( $result ) ) {
					...
					learn_press_add_message( sprintf( __( 'Bạn đã hoàn thành bài "%s" trong thời gian ngắn. Cố gắng phát huy nhé.', 'learnpress' ), $item->get_title() ) );
				} else {
