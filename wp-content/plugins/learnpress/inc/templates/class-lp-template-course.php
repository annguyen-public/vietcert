...
class LP_Template_Course extends LP_Abstract_Template {
	...
	public function course_purchase_button() {
		...
		try {
			if ( ! $user || ! $course ) {
				throw new Exception( 'Người dùng hoặc khóa học không tồn tại' );
			}

			if ( ! $user->can_purchase_course( $course->get_id() ) ) {
				throw new Exception( 'Bạn không thể mua khóa học này' );
			}
		} catch ( Throwable $e ) {
			$can_show = false;
		}
	...
	public function course_enroll_button() {
		...
		try {
			...
			if ( ! $user->can_enroll_course( $course->get_id() ) ) {
				throw new Exception( 'Bạn không thể đăng ký khóa học' );
			}

			if ( $user->has_finished_course( $course->get_id() ) ) {
				throw new Exception( 'Khóa học đã hoàn thành' );
	...
	public function course_extra_requirements( $course_id ) {
		...
		learn_press_get_template(
			...
			array(
				'type'    => 'requirements',
				'title'   => esc_html__( 'Yêu cầu', 'learnpress' ),
	...
	public function course_extra_key_features( $course_id ) {
		...
		learn_press_get_template(
			...
			array(
				'type'    => 'key-features',
				'title'   => esc_html__( 'Các điểm chính', 'learnpress' ),
	...
	public function course_extra_target_audiences( $course_id ) {
		...
		learn_press_get_template(
			...
			array(
				...
				'title'   => esc_html__( 'Học viên mục tiêu', 'learnpress' ),
	...
	public function course_continue_button() {
		...
		try {
			if ( ! $user || ! $course ) {
				throw new Exception( 'Người dùng hoặc khóa học không tồi tại!' );
			}

			if ( ! $user->has_enrolled_course( $course->get_id() ) ) {
				throw new Exception( 'Bạn chưa đăng ký khóa học' );
			}
	...
	public function count_object() {
		...
		$counts = apply_filters(
			...
			array(
				'lesson'  => sprintf(
					'<span class="meta-number">' . _n( '%d bài học', '%d bài học', $lessons, 'learnpress' ) . '</span>',
					$lessons
				),
				'quiz'    => sprintf(
					'<span class="meta-number">' . _n( '%d bài tập', '%d bài tập', $quizzes, 'learnpress' ) . '</span>',
					$quizzes
				),
				'student' => sprintf(
					'<span class="meta-number">' . _n( '%d học viên', '%d học viên', $students, 'learnpress' ) . '</span>',
					$students
				),
			),
	...
	public function course_extra_boxes() {
		...
		$boxes  = apply_filters(
			...
			array(
				array(
					'title' => __( 'Yêu cầu', 'learnpress' ),
					'items' => $course->get_extra_info( 'requirements' ),
				),
				array(
					'title' => __( 'Đặc điểm', 'learnpress' ),
					'items' => $course->get_extra_info( 'key_features' ),
				),
				array(
					'title' => __( 'Học viên mục tiêu', 'learnpress' ),
	...
	public function course_readmore() {
		...
		<div class="course-readmore">
			<a href="<?php the_permalink(); ?>"><?php echo esc_html__( 'Tìm hiểu thêm', 'learnpress' ); ?></a>
		</div>
		<?php
	}
