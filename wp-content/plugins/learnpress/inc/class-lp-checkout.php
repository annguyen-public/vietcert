...
class LP_Checkout {
	...
	public function get_checkout_fields() {
		if ( ! $this->verify_nonce() ) {
			$this->checkout_fields['invalid_request'] = new WP_Error( 'invalid_request', __( 'Phiên làm việc đã kết thúc.', 'learnpress' ) );
		} else {
			if ( $this->checkout_action === 'checkout-register' ) {
				$this->checkout_fields['reg_email']     = esc_html__( 'Email', 'learnpress' );
				$this->checkout_fields['reg_username']  = esc_html__( 'Tên đăng nhập', 'learnpress' );
				$this->checkout_fields['reg_password']  = esc_html__( 'Mật khẩu', 'learnpress' );
				$this->checkout_fields['reg_password2'] = esc_html__( 'Nhắc lại mật khẩu', 'learnpress' );
			} elseif ( $this->checkout_action === 'checkout-login' ) {
				$this->checkout_fields['username'] = esc_html__( 'Tên đăng nhập', 'learnpress' );
				$this->checkout_fields['password'] = esc_html__( 'Mật khẩu', 'learnpress' );
		...
		try {
			...
			if ( $order ) {
				if ( is_wp_error( $order ) ) {
					throw new Exception( sprintf( __( 'Lỗi %d: Không thể tạo đơn hàng. Vui lòng thử lại.', 'learnpress' ), 401 ) );
				}
				...
				if ( is_wp_error( $order ) ) {
					throw new Exception( sprintf( __( 'Lỗi %d: Không thể tạo đơn hàng. Vui lòng thử lại.', 'learnpress' ), 400 ) );
				}
			...
			foreach ( $cart->get_items() as $item ) {
				...
				if ( ! $item_id ) {
					throw new Exception( sprintf( __( 'Lỗi %d: Không thể tạo đơn hàng. Vui lòng thử lại.', 'learnpress' ), 402 ) );
				}
			...
			if ( ! $order_id || is_wp_error( $order_id ) ) {
				learn_press_add_message( __( 'Không thể thanh toán. Lỗi tạo đơn hàng.', 'learnpress' ) );
			}
		...
		switch ( $this->checkout_action ) {
			case 'checkout-register':
				if ( $name === 'reg_username' ) {
					if ( empty( $_POST['reg_username'] ) ) {
						return new WP_Error( 'username_empty', sprintf( __( '%s là bắt buộc nhập.', 'learnpress' ), $field ) );
					} elseif ( username_exists( $_POST['reg_username'] ) ) {
						return new WP_Error( 'username_exists', sprintf( __( '%s đã tồn tại.', 'learnpress' ), $_POST['reg_username'] ) );
					} elseif ( ! validate_username( $_POST['reg_username'] ) ) {
						return new WP_Error( 'username_invalid', sprintf( __( '%s không phải là tên đăng nhập hợp lệ.', 'learnpress' ), $_POST['reg_username'] ) );
					}
				} elseif ( $name === 'reg_email' ) {
					if ( empty( $_POST['reg_email'] ) ) {
						return new WP_Error( 'email_empty', sprintf( __( '%s là bắt buộc nhập.', 'learnpress' ), $field ) );
					} elseif ( email_exists( $_POST['reg_email'] ) ) {
						return new WP_Error( 'email_exists', sprintf( __( '%s đã tồn tại.', 'learnpress' ), $_POST['reg_email'] ) );
					} elseif ( ! is_email( $_POST['reg_email'] ) ) {
						return new WP_Error( 'email_invalid', sprintf( __( '%s không phải là email hợp lệ.', 'learnpress' ), $_POST['reg_email'] ) );
					}
				} elseif ( $name === 'reg_password' ) {
					if ( empty( $_POST['reg_password'] ) ) {
						return new WP_Error( 'password_empty', sprintf( __( '%s là bắt buộc nhập.', 'learnpress' ), $field ) );
					}
				} elseif ( $name === 'reg_password2' ) {
					if ( empty( $_POST['reg_password2'] ) ) {
						return new WP_Error( 'password2_empty', sprintf( __( '%s là bắt buộc nhập.', 'learnpress' ), $field ) );
					}
				}
			...
			case 'checkout-login':
				if ( $name === 'username' ) {
					...
					if ( ! username_exists( $_POST['username'] ) && ! email_exists( $_POST['username'] ) ) {
						return new WP_Error( 'username_exists', sprintf( __( '%s không tồn tại.', 'learnpress' ), $field ) );
					}
			...
			case 'guest-checkout':
				if ( empty( $_POST['guest_email'] ) ) {
					...
					return new WP_Error( 'email_invalid', __( 'Email của bạn không hợp lệ.', 'learnpress' ) );
				}
		...
		if ( $fields ) {
			foreach ( $fields as $name => $field ) {
				...
				if ( is_wp_error( $error ) ) {
					...
					$this->errors[ $name ] = new WP_Error( 'invalid_field', __( 'Dữ liệu không hợp lệ', 'learnpress' ) );
				}
			}
		}
		...
		if ( $cart->needs_payment() ) {
			if ( ! $this->payment_method instanceof LP_Gateway_Abstract ) {
				...
				if ( ! isset( $available_gateways[ $this->payment_method ] ) ) {
					$this->payment_method = '';
					throw new Exception( __( 'Chưa chọn phương pháp thanh toán', 'learnpress' ), LP_ERROR_NO_PAYMENT_METHOD_SELECTED );
				} else {
	...
	public function process_checkout() {
		...
		try {
			...
			if ( $cart->is_empty() ) {
				throw new Exception( __( 'Giỏ hàng rỗng.', 'learnpress' ) );
			}
			...
			foreach ( $cart->get_items() as $item ) {
				...
				if ( ! in_array( $item_type, learn_press_get_item_types_can_purchase() ) ) {
					throw new Exception( __( 'Loại sản phẩm không hợp lệ!', 'learnpress' ) );
				}
			}
