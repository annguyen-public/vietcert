...
function learn_press_section_item_types() {
	$types = array(
		'lp_lesson' => esc_html__( 'Bài học', 'learnpress' ),
		'lp_quiz'   => esc_html__( 'Bài tập', 'learnpress' ),
	);
...
function learn_press_get_course_duration_support() {
	return apply_filters(
		...
		array(
			'minute' => esc_html__( 'Phút', 'learnpress' ),
			'hour'   => esc_html__( 'Giờ', 'learnpress' ),
			'day'    => esc_html__( 'Ngày', 'learnpress' ),
			'week'   => esc_html__( 'Tuần', 'learnpress' ),
		)
	);
}
...
function learn_press_number_to_string_time( $number ) {
	...
	if ( preg_match( '!([0-9.]+) (minute|hour|day|week)!', $number, $matches ) ) {
		switch ( $matches[2] ) {
			case 'hour':
				$minute = $matches[1] * 60;
				$str    = sprintf( '%s giờ %s phút', absint( $minute / 60 ), $minute % 60 );
				break;
			case 'day':
				$hour = $matches[1] * 24;
				$str  = sprintf( '%s ngày %s giờ', absint( $hour / 24 ), $hour % 24 );
				break;
			case 'week':
				$day = $matches[1] * 7;
				$str = sprintf( '%s tuần %s ngày', absint( $day / 7 ), $day % 7 );
				break;
		}
	}
...
function learn_press_seconds_to_weeks( int $secs = 0 ) {
	...
	$result = '';
	if ( $weeks ) {
		$result .= sprintf( _n( '%s tuần', '%s tuần', $weeks, 'learnpress' ), $weeks ) . ' ';
	}

	if ( $days ) {
		$result .= sprintf( _n( '%s ngày', '%s ngày', $days, 'learnpress' ), $days ) . ' ';
	}

	if ( ! $weeks ) {
		if ( $hours ) {
			$result .= sprintf( _n( '%s giờ', '%s giờ', $hours, 'learnpress' ), $hours ) . ' ';
		}

		if ( $mins ) {
			$result .= sprintf( _n( '%s phút', '%s phút', $mins, 'learnpress' ), $mins ) . ' ';
		}
	}
