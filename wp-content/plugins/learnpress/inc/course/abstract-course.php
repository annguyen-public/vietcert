	...
	abstract class LP_Abstract_Course extends LP_Abstract_Post_Data {
		...
		public function get_course_price_html() {
			...
			if ( $this->is_free() ) {
				...
				$price_html .= apply_filters(
					...
					sprintf( '<span class="free">%s</span>', esc_html__( 'Miễn phí', 'learnpress' ) ),
					$this
				);
			} else {
				if ( $this->has_sale_price() ) {
		...
		public function get_price_html() {
			...
			if ( $this->is_free() ) {
				$price_html = apply_filters(
					'learn_press_course_price_html_free',
					esc_html__( 'Miễn phí', 'learnpress' ),
					$this
				);
			} else {
		...
		public function get_user_duration_html( $user_id = 0 ) {
			...
			if ( $course_info ) {
				...
				if ( $start_time + $duration > $now ) {
					...
					$html   = sprintf( __( 'Khóa học này sẽ hết hạn trong vòng %s', 'learnpress' ), $remain );
				}
			}
		...
		public function get_external_link_text() {
			return apply_filters( 'learn-press/course-external-link-text', _x( 'Thông tin thêm', 'External Link button text', 'learnpress' ), $this->get_id() );
		}
