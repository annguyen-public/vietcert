	...
	function learn_press_get_course_tabs() {
		...
		if ( isset( $_GET['preview'] ) || $course && $course->get_content() ) {
			$defaults['overview'] = array(
				'title'    => esc_html__( 'Mô tả khóa học', 'learnpress' ),
				'priority' => 10,
		...
		$defaults['curriculum'] = array(
			'title'    => esc_html__( 'Chương trình giảng dạy', 'learnpress' ),
			'priority' => 30,
			'callback' => LP()->template( 'course' )->func( 'course_curriculum' ),
		);

		$defaults['instructor'] = array(
			'title'    => esc_html__( 'Giáo viên', 'learnpress' ),
			'priority' => 40,
	...
	function learn_press_single_document_title_parts( $title ) {
		if ( learn_press_is_course() ) {
			...
			if ( learn_press_is_search() ) {
				$title['title'] = esc_html__( 'Kết quả tìm kiếm khóa học', 'learnpress' );
			} else {
				$title['title'] = esc_html__( 'Khóa học', 'learnpress' );
			}
...
if ( ! function_exists( 'learn_press_item_meta_type' ) ) {
	function learn_press_item_meta_type( $course, $item ) {
		...
		<?php if ( $item->post_type == 'lp_quiz' ) { ?>
			...
			<span class="lp-label lp-label-quiz"><?php _e( 'Bài tập', 'learnpress' ); ?></span>

			<?php if ( $course->final_quiz == $item->ID ) { ?>

				<span class="lp-label lp-label-final"><?php _e( 'Cuối kỳ', 'learnpress' ); ?></span>

			<?php } ?>

		<?php } elseif ( $item->post_type == 'lp_lesson' ) { ?>

			<span class="lp-label lp-label-lesson"><?php _e( 'Bài học', 'learnpress' ); ?></span>
			<?php if ( get_post_meta( $item->ID, '_lp_preview', true ) == 'yes' ) { ?>

				<span class="lp-label lp-label-preview"><?php _e( 'Xem trước', 'learnpress' ); ?></span>
...
function learn_press_quiz_meta_final( $item ) {
	...
	echo '<span class="item-meta final-quiz">' . esc_html__( 'Cuối kỳ', 'learnpress' ) . '</span>';
}
		...
		switch ( strtolower( $duration_text ) ) {
			case 'minute':
				$duration_str = sprintf(
					_n( '%s phút', '%s phút', $duration_number, 'learnpress' ),
					$duration_number
				);
				break;
			case 'hour':
				$duration_str = sprintf(
					_n( '%s giờ', '%s giờ', $duration_number, 'learnpress' ),
					$duration_number
				);
				break;
			case 'day':
				$duration_str = sprintf( _n( '%s ngày', '%s ngày', $duration_number, 'learnpress' ), $duration_number );
				break;
			case 'week':
				$duration_str = sprintf(
					_n( '%s tuần', '%s tuần', $duration_number, 'learnpress' ),
					$duration_number
				);
				break;
			default:
...
function learn_press_get_post_level( $post_id ) {
	...
	return apply_filters(
		...
		! empty( $level ) ? lp_course_level()[ $level ] : esc_html__( 'Tất các mức độ', 'learnpress' ),
		$post_id
	);
}
...
function lp_course_level() {
	return apply_filters(
		...
		array(
			''             => esc_html__( 'Tất cả mức độ', 'learnpress' ),
			'beginner'     => esc_html__( 'Sơ cấp', 'learnpress' ),
			'intermediate' => esc_html__( 'Trung cấp', 'learnpress' ),
			'expert'       => esc_html__( 'Cao cấp', 'learnpress' ),
		)
	);
}
