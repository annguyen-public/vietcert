...
if ( ! class_exists( 'LP_Profile' ) ) {
	...
	class LP_Profile {
		...
		public function get_tabs() {
			if ( $this->_tabs === null ) {
				...
				$this->_default_settings = array(
					'courses'       => array(
						'title'    => esc_html__( 'Khóa học', 'learnpress' ),
					...
					'quizzes'       => array(
						'title'    => esc_html__( 'Bài tập', 'learnpress' ),
					...
					'orders'        => array(
						'title'    => esc_html__( 'Đơn hàng', 'learnpress' ),
					...
					'order-details' => array(
						'title'    => esc_html__( 'Chi tiết đơn hàng', 'learnpress' ),
					...
					'settings'      => array(
						...
						'sections' => array(
							...
							'change-password'   => array(
								'title'    => esc_html__( 'Mật khẩu', 'learnpress' ),
					...
					'logout'        => array(
						'title'    => esc_html__( 'Đăng xuất', 'learnpress' ),
				...
				if ( $this->is_enable_avatar() ) {
					$this->_default_settings['settings']['sections']['avatar'] = array(
						'title'    => esc_html__( 'Ảnh đại hiện', 'learnpress' ),
						'callback' => array( $this, 'tab_order_details' ),
				...
				if ( 'yes' === self::get_option_publish_profile() ) {
					$this->_default_settings['settings']['sections']['privacy'] = array(
						'title'    => esc_html__( 'Riêng tư', 'learnpress' ),
		...
		public function get_privacy_settings() {
			$privacy = array(
				array(
					'name'        => esc_html__( 'Khóa học', 'learnpress' ),
					'id'          => 'courses',
					'default'     => 'yes',
					'type'        => 'yes-no',
					'description' => esc_html__( 'Công khai hồ sơ khóa học của bạn.', 'learnpress' ),
				),
				array(
					'name'        => esc_html__( 'Bài tập', 'learnpress' ),
					'id'          => 'quizzes',
					'default'     => 'yes',
					'type'        => 'yes-no',
					'description' => esc_html__( 'Công khai thông tin bài tập.', 'learnpress' ),
				),
			);
		...
		public function get_own_courses_filters( $current_filter = '' ) {
			...
			$defaults = array(
				'all'     => sprintf( '<a href="%s">%s</a>', esc_url( $url ), esc_html__( 'Tất cả', 'learnpress' ) ),
				'publish' => sprintf( '<a href="%s">%s</a>', esc_url( add_query_arg( 'filter-status', 'publish', $url ) ), esc_html__( 'Đã đăng', 'learnpress' ) ),
				'pending' => sprintf( '<a href="%s">%s</a>', esc_url( add_query_arg( 'filter-status', 'pending', $url ) ), esc_html__( 'Đang chờ', 'learnpress' ) ),
			);
		...
		public function get_purchased_courses_filters( $current_filter = '' ) {
			...
			$defaults = array(
				...
				'finished'     => sprintf( '<a href="%s">%s</a>', esc_url( add_query_arg( 'filter-status', 'finished', $url ) ), __( 'Đã kết thúc', 'learnpress' ) ),
				'passed'       => sprintf( '<a href="%s">%s</a>', esc_url( add_query_arg( 'filter-status', 'passed', $url ) ), __( 'Đậu', 'learnpress' ) ),
				'failed'       => sprintf( '<a href="%s">%s</a>', esc_url( add_query_arg( 'filter-status', 'failed', $url ) ), __( 'Rớt', 'learnpress' ) ),
				'not-enrolled' => sprintf( '<a href="%s">%s</a>', esc_url( add_query_arg( 'filter-status', 'not-enrolled', $url ) ), __( 'Chưa đăng ký', 'learnpress' ) ),
			);
		...
		public function get_quizzes_filters( $current_filter = '' ) {
			$url      = $this->get_current_url( false );
			$defaults = array(
				'all'       => sprintf( '<a href="%s">%s</a>', esc_url( $url ), __( 'Tất cả', 'learnpress' ) ),
				'completed' => sprintf( '<a href="%s">%s</a>', esc_url( add_query_arg( 'filter-status', 'completed', $url ) ), __( 'Đã kết thúc', 'learnpress' ) ),
				'passed'    => sprintf( '<a href="%s">%s</a>', esc_url( add_query_arg( 'filter-graduation', 'passed', $url ) ), __( 'Đậu', 'learnpress' ) ),
				'failed'    => sprintf( '<a href="%s">%s</a>', esc_url( add_query_arg( 'filter-graduation', 'failed', $url ) ), __( 'Rớt', 'learnpress' ) ),
			);
		...
		public function get_profile_picture( $type = '', $size = 96 ) {
			...
			$avatar = get_avatar( $user->get_id(), $size, '', esc_attr__( 'Hình đại hiện người dùng', 'learnpress' ), $args );
