...
class LP_REST_Profile_Controller extends LP_Abstract_REST_Controller {
	...
	public function upload_avatar( WP_REST_Request $request ) {
		...
		try {
			...
			$response->message = __( 'Đã cập nhật ảnh đại diện', 'learnpress' );
		} catch ( \Throwable $th ) {
		...
		try {
			...
			if ( $path_img ) {
				...
				if ( file_exists( $path ) ) {
					...
					$response->status  = 'success';
					$response->message = esc_html__( 'Ảnh đại diện đã được xóa thành công', 'learnpress' );
				}
			}
		} catch ( \Throwable $th ) {
			$response->message = $th->getMessage();
	...
	public function course_tab( $request ) {
		...
		try {
			...
			if ( empty( $course_item_objects ) ) {
				throw new Exception( esc_html__( 'Không có khóa học nào!', 'learnpress' ) );
			}
			...
			if ( empty( $course_ids ) ) {
				throw new Exception( esc_html__( 'Không có khóa học nào khả dụng!', 'learnpress' ) );
			}
