...
if ( ! $content ) {
	$message = esc_html__( 'Chưa có nội dung bài học.', 'learnpress' );

	if ( $lesson->current_user_can_edit() ) {
		$message .= sprintf( '<a href="%s" class="edit-content">%s</a>', $lesson->get_edit_link(), esc_html__( 'Chỉnh sửa', 'learnpress' ) );
	}
