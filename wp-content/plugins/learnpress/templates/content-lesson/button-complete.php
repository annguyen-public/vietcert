...
$message_confirm_complete_item = sprintf( '%s "%s"?', __( 'Bạn có muốn kết thúc bài học', 'learnpress' ), $item->get_title() );
...
if ( $completed ) :
	...
	<div>
		...
		echo sprintf(
			'%s %s',
			esc_html__( 'Bạn đã hoàn thành bài học này vào ', 'learnpress' ),
			$user->get_item_data( $item->get_id(), $course->get_id(), 'end_time' )
		)
		?>
	</div>
	<button class="lp-button completed" disabled>
		<i class="fa fa-check"></i><?php esc_html_e( 'Đã hoàn thành', 'learnpress' ); ?>
	</button>
<?php else : ?>
	...
	<form method="post" name="learn-press-form-complete-lesson"
		  ...
		  data-title="<?php echo esc_attr(__('Kết thúc bài học', 'learnpress'))?>"
		...
		<button class="lp-button button button-complete-item button-complete-lesson lp-btn-complete-item">
			<?php echo esc_html__( 'Hoàn thành bài học', 'learnpress' ); ?>
		</button>
