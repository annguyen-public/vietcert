...
if ( ! $query_orders['items'] ) {
	learn_press_display_message( __( 'Không có thông tin đặt hàng!', 'learnpress' ) );
	return;
}
?>

<h3 class="profile-heading"><?php esc_html_e( 'Đơn hàng của tôi', 'learnpress' ); ?></h3>

<table class="lp-list-table profile-list-orders profile-list-table">
	<thead>
		<tr class="order-row">
			<th class="column-order-number"><?php esc_html_e( 'Đơn hàng', 'learnpress' ); ?></th>
			<th class="column-order-total"><?php esc_html_e( 'Tổng', 'learnpress' ); ?></th>
			<th class="column-order-status"><?php esc_html_e( 'Trạng thái', 'learnpress' ); ?></th>
			<th class="column-order-date"><?php esc_html_e( 'Ngày', 'learnpress' ); ?></th>
			<th class="column-order-actions"><?php esc_html_e( 'Hành động', 'learnpress' ); ?></th>
		</tr>
	</thead>
