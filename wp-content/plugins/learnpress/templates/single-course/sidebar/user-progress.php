...
<div class="course-results-progress">
	...
	if ( $course_results['items']['lesson']['total'] ) :
		...
		<div class="items-progress">
			<h4 class="items-progress__heading">
				<?php esc_html_e( 'Bài học đã hoàn thành:', 'learnpress' ); ?>
			</h4>
	...
	if ( $course_results['items']['quiz']['total'] ) :
		...
		<div class="items-progress">
			<h4 class="items-progress__heading">
				<?php esc_html_e( 'Bài tập đã hoàn thành:', 'learnpress' ); ?>
			</h4>
			<span class="number" title="<?php esc_attr( sprintf( __( 'Chưa đạt %1$d, Đạt %2$d', 'learnpress' ), $quiz_false, $course_results['items']['quiz']['passed'] ) ); ?>"><?php printf( __( '%1$d/%2$d', 'learnpress' ), $course_results['items']['quiz']['completed'], $course_results['items']['quiz']['total'] ); ?></span>
		</div>
	<?php endif; ?>
	...
	<div class="course-progress">
		<h4 class="items-progress__heading">
			<?php esc_html_e( 'Tiến trình học:', 'learnpress' ); ?>
		</h4>
		...
		<div class="learn-press-progress lp-course-progress <?php echo $course_data->is_passed() ? ' passed' : ''; ?>"
			...
			<div class="lp-passing-conditional" data-content="<?php printf( esc_html__( 'Điều kiện hoàn thành: %s%%', 'learnpress' ), $passing_condition ); ?>" style="left: <?php echo $passing_condition; ?>%;">
			</div>
		</div>
	</div>
