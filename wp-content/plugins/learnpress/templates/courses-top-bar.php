...
<div class="lp-courses-bar <?php echo esc_attr( $active ); ?>">
	<form class="search-courses" method="get" action="<?php echo esc_url( home_url( '/' ) ); ?>">
		...
		<input type="text" placeholder="<?php esc_attr_e( 'Tìm khóa học...', 'learnpress' ); ?>" name="s" value="<?php echo esc_attr( $s ); ?>">
	...
	<div class="switch-layout">
		<?php foreach ( $layouts as $layout => $value ) : ?>
			...
			<label class="switch-btn <?php echo $layout; ?>" title="<?php echo sprintf( esc_attr__( 'Chuyển sang %s', 'learnpress' ), $layout ); ?>" for="lp-switch-layout-btn-<?php echo esc_attr( $layout ); ?>"></label>
		<?php endforeach; ?>
	</div>
</div>
