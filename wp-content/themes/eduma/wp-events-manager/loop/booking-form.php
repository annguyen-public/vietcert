...
?>
<h3 class="book-title"><?php esc_html_e( 'Mua vé', 'eduma' ); ?></h3>
...
<div class="event_register_area">
    ...
    <form name="event_register" class="event_register" method="POST">
        ...
        <ul class="info-event">
            <li>
                <div class="label"><?php esc_html_e( 'Tổng vé', 'eduma' ); ?></div>
                <div class="value"><?php echo absint( $event->qty ); ?></div>
            </li>
            <li>
                <div class="label"><?php esc_html_e( 'Vé đã mua', 'eduma' ); ?></div>
                <div class="value"><?php echo esc_html( $event->booked_quantity() ); ?></div>
            </li>
            <li class="event-cost">
                <div class="label"><?php esc_html_e( 'Phí', 'eduma' ); ?></div>
                <div class="value"><?php echo ( $event->get_price() ) ? wpems_format_price( $event->get_price() ) . esc_html__( '/vé', 'eduma' ) : '<span class="free">' . esc_html__( 'Miễn phí', 'eduma' ) . '</span>'; ?></div>
            </li>
            <li>
				<?php if ( $user_reg == 0 && $event->is_free() && wpems_get_option( 'email_register_times' ) === 'once' ) : ?>
                    <div class="label"><?php esc_html_e( 'Số lượng', 'eduma' ); ?></div>
                    <div class="value">
			...
			<?php if ( intval( $event->get_price() ) > 0 ) : ?>
                ...
                <li class="event-payment">
                    <div class="label"><?php esc_html_e( 'Thanh toán bằng', 'eduma' ); ?></div>
        ...
        <?php if(empty($payments)) { ?>
            <p class="event_auth_register_message_error">
                <?php echo esc_html__( 'Vui lòng cài đặt phương thức thanh toán', 'eduma' ); ?>
            </p>
        <?php }?>
        ...
        <div class="event_register_foot">
			...
			<?php if ( $is_expired ) : ?>
                <button type="submit" disabled class="event_button_disable"><?php esc_html_e( 'Hết hạn', 'eduma' ); ?></button>
			<?php elseif ( absint( $event->qty ) == 0 ) : ?>
                <button type="submit" disabled class="event_button_disable"><?php esc_html_e( 'Hết vé', 'eduma' ); ?></button>
			<?php else : ?>
				<?php if ( ! is_user_logged_in() ) { ?>
					<a href="<?php echo esc_url( add_query_arg( 'redirect_to', urlencode( get_permalink() ), get_permalink( get_option( 'thimpress_events_login_page_id' ) ) ) ) ?>" class="event_register_submit event_auth_button js-show-popup"><?php esc_html_e( 'Đăng nhập ngay', 'eduma' ); ?></a>
                    <p></p>
                    <p class="login-notice">
						<?php echo esc_html__( 'Phải đăng nhập để đăng ký tham gia sự kiện!', 'eduma' ); ?>
                    </p>
				<?php } else { ?>
                    <button type="submit" class="event_register_submit event_auth_button" <?php echo $payments ? '' : 'disabled="disabled"' ?>><?php esc_html_e( 'Đăng ký ngay', 'eduma' ); ?></button>
				<?php } ?>
			<?php endif ?>
