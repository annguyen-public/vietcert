...
if ( $total == 0 ) {
	$message = '<p class="message message-error">' . esc_html__( 'Không có khóa học nào phù hợp!', 'eduma' ) . '</p>';
	$index   = esc_html__( 'Không có khóa học nào!', 'eduma' );
} elseif ( $total == 1 ) {
...
$default_order = apply_filters( 'thim_default_order_course_option', array(
	'newly-published' => esc_html__( 'Mới nhất', 'eduma' ),
	'alphabetical'    => esc_html__( 'Từ A-Z', 'eduma' ),
	'most-members'    => esc_html__( 'Nhiều người học', 'eduma' )
) );;
	...
	<div class="thim-course-top switch-layout-container <?php if ( $show_desc && $cat_desc ) {
		...
		<div class="courses-searching">
			<form method="get" action="<?php echo esc_url( get_post_type_archive_link( 'lp_course' ) ); ?>">
				<input type="text" value="" name="s" placeholder="<?php esc_attr_e( 'Nhập tên khóa học cần tìm', 'eduma' ) ?>" class="form-control course-search-filter" autocomplete="off" />
