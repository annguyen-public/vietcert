...
//		}
		...
		<ul class="entry-meta">
			...
			if ( isset( $theme_options_data['thim_show_author'] ) && $theme_options_data['thim_show_author'] == 1 ) {
				?>
				<li class="author">
					<span><?php echo esc_html__( 'Đăng bởi', 'eduma' ); ?></span>
			...
			if ( isset( $theme_options_data['thim_show_category'] ) && $theme_options_data['thim_show_category'] == 1 && get_the_category() ) {
				...
				<li class="entry-category">
					<span><?php esc_html_e( 'Phân loại', 'eduma' ); ?></span> <?php the_category( ', ', '' ); ?>
				</li>
				<?php
			}
			if ( isset( $theme_options_data['thim_show_date'] ) && $theme_options_data['thim_show_date'] == 1 && ( is_single() || ( $archive_layout == 'grid') ) ) {
				?>
				<li class="entry-date">
					<span><?php echo esc_html__( 'Ngày đăng', 'eduma' ); ?></span>
			...
			if ( isset( $theme_options_data['thim_show_comment'] ) && $theme_options_data['thim_show_comment'] == 1 ) {
				...
				<?php if ( !post_password_required() && ( comments_open() || '0' != get_comments_number() ) ) :
					...
					<li class="comment-total">
						<span><?php echo esc_html__( 'Bình luận', 'eduma' ) ?></span>
						<?php comments_popup_link( esc_html__( '0 lượt', 'eduma' ), esc_html__( '1 lượt', 'eduma' ), '% ' . esc_html__( 'lượt', 'eduma' ) ); ?>
					</li>
					<?php
				endif;
			}
	...
	function thim_entry_footer() {
		...
		if ( 'post' == get_post_type() ) {
			...
			if ( $categories_list && thim_categorized_blog() ) {
				printf( '<span class="cat-links">' . esc_html__( 'Đăng trong %1$s', 'eduma' ) . '</span>', $categories_list );
			}
			/* translators: used between list items, there is a space after the comma */
		...
		if ( !is_single() && !post_password_required() && ( comments_open() || get_comments_number() ) ) {
			...
			comments_popup_link( esc_html__( 'Để lại bình luận', 'eduma' ), esc_html__( '1 lượt', 'eduma' ), '%' . esc_html__( 'lượt', 'eduma' ) );
			echo '</span>';
		}

		edit_post_link( esc_html__( 'Chỉnh sửa', 'eduma' ), '<span class="edit-link">', '</span>' );
	}
...
if ( !function_exists( 'thim_archive_title' ) ) :
	...
	function thim_archive_title( $before = '', $after = '' ) {
		...
		if ( is_category() ) {
			...
			$title = sprintf( esc_html__( 'Năm: %s', 'eduma' ), get_the_date( _x( 'Y', 'yearly archives date format', 'eduma' ) ) );
		} elseif ( is_month() ) {
			$title = sprintf( esc_html__( 'Tháng: %s', 'eduma' ), get_the_date( _x( 'F Y', 'monthly archives date format', 'eduma' ) ) );
		} elseif ( is_day() ) {
			$title = sprintf( esc_html__( 'Ngày: %s', 'eduma' ), get_the_date( _x( 'F j, Y', 'daily archives date format', 'eduma' ) ) );
			...
			$title = sprintf( esc_html__( 'Kết quả tìm kiếm cho: %s', 'eduma' ), get_search_query() );
		} else {
		...
		if ( function_exists( 'is_bbpress' ) ) {
			...
			if ( bbp_is_search() ) {
				...
				if ( current_user_can( 'manage_topic_tags' ) ) {
					$tag_data[] = '' . esc_html__( '(Chỉnh sửa)', 'eduma' ) . '';
				}
				...
				$pre_current_text = esc_html__( 'Chỉnh sửa', 'eduma' );
			} else {
