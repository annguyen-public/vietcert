						...
						<div class="thim-login-container">
							...
							<div class="thim-popup-inner">
								<div class="thim-login">
									<h4 class="title"><?php esc_html_e( 'Đăng nhập', 'eduma' ); ?></h4>
									<form name="loginpopopform"
										...
										<p class="login-username">
											<input type="text" name="log"
												   placeholder="<?php esc_html_e( 'Tên đăng nhập hoặc email', 'eduma' ); ?>"
												   class="input required" value="" size="20"/>
										</p>
										<p class="login-password">
											<input type="password" name="pwd"
												   placeholder="<?php esc_html_e( 'Mật khẩu', 'eduma' ); ?>"
												   class="input required" value="" size="20"/>
										</p>
										...
										<?php echo '<a class="lost-pass-link" href="' . thim_get_lost_password_url() . '" title="' . esc_attr__( 'Quên mật khẩu', 'eduma' ) . '">' . esc_html__( 'Quên mật khẩu?', 'eduma' ) . '</a>'; ?>
										<p class="forgetmenot login-remember">
											<label for="popupRememberme"><input name="rememberme" type="checkbox"
																				value="forever"
																				id="popupRememberme"/> <?php esc_html_e( 'Lưu đăng nhập', 'eduma' ); ?>
											</label></p>
										<p class="submit login-submit">
											<input type="submit" name="wp-submit"
												   class="button button-primary button-large"
												   value="<?php esc_attr_e( 'Đăng nhập', 'eduma' ); ?>"/>
									...
									if ( $registration_enabled ) {
										echo '<p class="link-bottom">' . esc_html__( 'Chưa có tài khoản? ', 'eduma' ) . ' <a class="register" href="' . esc_url( thim_get_register_url() ) . '">' . esc_html__( 'Đăng ký ngay', 'eduma' ) . '</a></p>';
									}
									?>
								</div>
								...
								<?php if ( $registration_enabled ): ?>
									<div class="thim-register">
										<?php
										if ( class_exists( 'LearnPress' ) && $lp_enable_register_profile == 'yes' ) {
											learn_press_get_template( 'global/form-register.php' );
										} else {
											?>
											<h4 class="title"><?php echo esc_html_x( 'Đăng ký tài khoản mới', 'Login popup form', 'eduma' ); ?></h4>
 											<form class="<?php if ( get_theme_mod( 'thim_auto_login', true ) ) {
												...
												<p>
													<input placeholder="<?php esc_attr_e( 'Tên đăng nhập', 'eduma' ); ?>"
												...
												<?php if ( get_theme_mod( 'thim_auto_login', true ) ) { ?>
													<p>
														<input placeholder="<?php esc_attr_e( 'Mật khẩu', 'eduma' ); ?>"
															   type="password" name="password" class="input required"/>
													</p>
													<p>
														<input
															placeholder="<?php esc_attr_e( 'Nhắc lại mật khẩu', 'eduma' ); ?>"
												...
												<?php if ( ! empty( $instance['term'] ) ): ?>
													<p>
														...
														<label
															for="termFormFieldPopup"><?php printf( __( 'Tôi đã đọc và chấp nhận <a href="%s" target="_blank">điều khoản sử dụng</a>', 'eduma' ), esc_url( $instance['term'] ) ); ?></label>
													</p>
												<?php endif; ?>
												...
												<p class="submit">
													<input type="submit" name="wp-submit"
														   class="button button-primary button-large"
														   value="<?php echo esc_attr_x( 'Đăng ký', 'Login popup form', 'eduma' ); ?>"/>
												</p>
												<input type="hidden" name="redirect_to"
													   value="<?php echo esc_url( thim_eduma_get_current_url() ); ?>"/>
										...
										<?php } ?>
										<?php echo '<p class="link-bottom">' . esc_html_x( 'Đã có tài khoản? ', 'Login popup form', 'eduma' ) . ' <a class="login" href="' . esc_url( thim_get_login_page_url() ) . '">' . esc_html_x( 'Đăng nhập ngay', 'Login popup form', 'eduma' ) . '</a></p>'; ?>
