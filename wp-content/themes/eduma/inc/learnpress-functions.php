	...
	function thim_learnpress_page_title( $echo = true ) {
		...
		if ( get_post_type() == 'lp_course' && ! is_404() && ! is_search() || learn_press_is_courses() || learn_press_is_course_taxonomy() ) {
			if ( learn_press_is_course_taxonomy() ) {
				...
				$title = esc_html__( 'Tất cả khóa học', 'eduma' );
			}
		}

		if ( get_post_type() == 'lp_course' && is_search() ) {
			$title = esc_attr__( 'Kết quả tìm kiếm cho:', 'eduma' ) . ' ' . esc_attr( get_search_query() );
		}
		...
		if ( get_post_type() == 'lp_quiz' && ! is_404() && ! is_search() ) {
			if ( is_tax() ) {
				...
				$title = esc_html__( 'Bài tập', 'eduma' );
			}
		}
		if ( $echo ) {
			echo $title;
...
if ( ! function_exists( 'thim_learnpress_breadcrumb' ) ) {
	function thim_learnpress_breadcrumb() {
		...
		echo '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
  				<a itemprop="item" href="' . esc_url( get_home_url() ) . '" title="' . esc_attr__( 'Trang chủ', 'eduma' ) . '">
					 <span itemprop="name">' . esc_html__( 'Trang chủ', 'eduma' ) . '</span>
   				</a>
		...
		if ( is_single() ) {
			...
			if ( get_post_type() == 'lp_course' ) {
				// All courses
				echo '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
 						<a itemprop="item" href="' . esc_url( get_post_type_archive_link( 'lp_course' ) ) . '" title="' . esc_attr__( 'Tất cá khóa học', 'eduma' ) . '">
							<span itemprop="name">' . esc_html__( 'Tất cá khóa học', 'eduma' ) . '</span>
 						</a>
			...
			if ( learn_press_is_course_taxonomy() || learn_press_is_course_tag() ) {
				...
				if ( ! empty( $_REQUEST['s'] ) && ! empty( $_REQUEST['ref'] ) && ( $_REQUEST['ref'] == 'course' ) ) {
					...
					echo '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
						<a itemprop="item" href="' . esc_url( get_post_type_archive_link( 'lp_course' ) ) . '" title="' . esc_attr__( 'Tất cá khóa học', 'eduma' ) . '">
						<span itemprop="name">' . esc_html__( 'Tất cá khóa học', 'eduma' ) . '</span></a>
						<meta itemprop="position" content="2"/>
					</li>';

					// Search result
					echo '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
						<span itemprop="name" title="' . esc_attr__( 'Kết quả tìm kiếm cho:', 'eduma' ) . ' ' . esc_attr( get_search_query() ) . '">' . esc_html__( 'Kết quả tìm kiếm cho:', 'eduma' ) . ' ' . esc_html( get_search_query() ) . '</span>
						<meta itemprop="position" content="3"/></li>';
				} else {
					echo '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
							<span itemprop="name" title="' . esc_attr__( 'Tất cá khóa học', 'eduma' ) . '">' . esc_html__( 'Tất cá khóa học', 'eduma' ) . '</span>
...
if ( ! function_exists( 'thim_course_ratings_count' ) ) {
	function thim_course_ratings_count( $course_id = null ) {
		if ( ! class_exists( 'LP_Addon_Course_Review_Preload' ) || ! thim_is_version_addons_review( '3' ) ) {
			return;
		}
		if ( ! $course_id ) {
		...
		if ( $user->has_course_status( $course_id, array( 'enrolled', 'completed', 'finished' ) ) ) {
			if ( ! learn_press_get_user_rate( $course_id ) ) {
				...
				<div class="add-review">
					<h3 class="title"><?php esc_html_e( 'Để lại đánh giá', 'eduma' ); ?></h3>

					<p class="description"><?php esc_html_e( 'Vui lòng mô tả trải nghiệm của bạn.', 'eduma' ); ?></p>
					<?php do_action( 'learn_press_before_review_fields' ); ?>
					<form method="post">
						<div>
							<label for="review-title"><?php esc_html_e( 'Tiêu đề', 'eduma' ); ?>
								<span class="required">*</span></label>
							<input required type="text" id="review-title" name="review-course-title"/>
						</div>
						<div>

							<label><?php esc_html_e( 'Đánh giá', 'eduma' ); ?>
								<span class="required">*</span></label>
						...
						<div>
							<label for="review-content"><?php esc_html_e( 'Bình luận', 'eduma' ); ?>
								<span class="required">*</span></label>
						...
						<button type="submit"><?php esc_html_e( 'Gửi đánh giá', 'eduma' ); ?></button>
					</form>
...
if ( ! function_exists( 'thim_course_ratings' ) ) {
	function thim_course_ratings() {
		...
		<div class="course-review">
			<label><?php esc_html_e( 'Đánh giá', 'eduma' ); ?></label>
			<div class="value"<?php echo $_aggregateRating; ?>>
				<?php thim_print_rating( $course_rate ); ?>
				<span><?php $ratings ? printf( _n( '(%1$s đánh giá)', '(%1$s đánh giá)', $ratings, 'eduma' ), '<span ' . $_reviewCount . '>' . number_format_i18n( $ratings ) . '</span>' ) : printf( __( '(%1$s đánh giá)', 'eduma' ), '<span ' . $_reviewCount . '>0</span>' ); ?></span>
				<?php
				echo $rating_meta;
				?>
			</div>
...
if ( ! function_exists( 'thim_course_ratings_meta' ) ) {
	function thim_course_ratings_meta() {
		...
		<div class="course-review">
			...
			<div class="value">
				<?php echo $course_rate; ?> <?php esc_html_e( 'Sao', 'eduma' ); ?>
				<span><?php $ratings ? printf( _n( '(%1$s đánh giá)', '(%1$s đánh giá)', $ratings, 'eduma' ), number_format_i18n( $ratings ) ) : printf( __( '(%1$s đánh giá)', 'eduma' ), '<span itemprop="reviewCount">0</span>' ); ?></span>
			</div>
		</div>
		<?php
	}
...
function thim_item_quiz_meta_questions( $item ) {
	...
	echo '<span class="meta count-questions">' . sprintf( $count ? _n( '%d câu hỏi', '%d câu hỏi', $count, 'eduma' ) : __( '%d câu hỏi', 'eduma' ), $count ) . '</span>';
}
...
if ( ! function_exists( 'thim_item_meta_duration' ) ) {
	function thim_item_meta_duration( $item ) {
		...
		if ( is_a( $duration, 'LP_Duration' ) && $duration->get() ) {
			$format = array(
				'day'    => '%s ' . _x( 'ngày', 'duration', 'eduma' ),
				'hour'   => '%s ' . _x( 'giờ', 'duration', 'eduma' ),
				'minute' => '%s ' . _x( 'phút', 'duration', 'eduma' ),
				'second' => '%s ' . _x( 'giây', 'duration', 'eduma' ),
			);
	...
	function thim_course_certificate( $course_id ) {
		...
		if ( class_exists( 'LP_Addon_Certificates_Preload' ) && thim_is_version_addons_certificates( '3' ) ) {
			...
			<li class="cert-feature">
				...
				<span class="label"><?php esc_html_e( 'Chứng chỉ', 'eduma' ); ?></span>
				<span
					class="value"><?php echo ( get_post_meta( $course_id, '_lp_cert', true ) ) ? esc_html__( 'Có', 'eduma' ) : esc_html__( 'Không', 'eduma' ); ?></span>
			</li>
			<?php
		}
	}
...
if ( ! function_exists( 'thim_course_info' ) ) {
	function thim_course_info() {
		...
		<div class="thim-course-info">
			<h3 class="title"><?php esc_html_e( 'Thông tin khóa học', 'eduma' ); ?></h3>
			<ul>
				<li class="lectures-feature">
					<i class="fa fa-files-o"></i>
					<span class="label"><?php esc_html_e( 'Bài học', 'eduma' ); ?></span>
					<span
						class="value"><?php echo $course->get_curriculum_items( 'lp_lesson' ) ? count( $course->get_curriculum_items( 'lp_lesson' ) ) : 0; ?></span>
				</li>
				<li class="quizzes-feature">
					<i class="fa fa-puzzle-piece"></i>
					<span class="label"><?php esc_html_e( 'Bài kiểm tra', 'eduma' ); ?></span>
					<span
				...
				<?php if ( ! empty( $course_duration ) ): ?>
					<li class="duration-feature">
						...
						<span class="label"><?php esc_html_e( 'Thời gian', 'eduma' ); ?></span>
				...
				<?php if ( ! empty( $course_skill_level ) ): ?>
					<li class="skill-feature">
						...
						<span class="label"><?php esc_html_e( 'Mức độ', 'eduma' ); ?></span>
				...
				<?php if ( ! empty( $course_language ) ): ?>
					<li class="language-feature">
						...
						<span class="label"><?php esc_html_e( 'Ngôn ngữ', 'eduma' ); ?></span>
						<span class="value"><?php echo esc_html( $course_language ); ?></span>
					</li>
				<?php endif; ?>
				<li class="students-feature">
					<i class="fa fa-users"></i>
					<span class="label"><?php esc_html_e( 'Học viên', 'eduma' ); ?></span>
				...
				<li class="assessments-feature">
					...
					<span class="label"><?php esc_html_e( 'Đánh giá', 'eduma' ); ?></span>
					<span
						class="value"><?php echo ( get_post_meta( $course_id, '_lp_course_result', true ) == 'evaluate_lesson' ) ? esc_html__( 'Có', 'eduma' ) : esc_html__( 'Tự đánh giá', 'eduma' ); ?></span>
				</li>
			</ul>
			...
			<div class="thim-ralated-course <?php echo $cls_layout; ?>">
				...
				<?php if ( $style_content == 'new-1' ) { ?>
					<div class="sc_heading clone_title  text-left">
						<h2 class="title"><?php esc_html_e( 'Có thể bạn sẽ thích', 'eduma' ); ?></h2>
						<div class="clone"><?php esc_html_e( 'Có thể bạn sẽ thích', 'eduma' ); ?></div>
					</div>
				<?php } else { ?>
					<h3 class="related-title">
						<?php esc_html_e( 'Có thể bạn sẽ thích', 'eduma' ); ?>
					</h3>
				<?php } ?>
							...
							<article class="lpr_course">
								<div class="course-item">
									<div class="course-thumbnail">
										...
										<?php echo '<a class="course-readmore" href="' . esc_url( get_the_permalink( $course_item->ID ) ) . '">' . esc_html__( 'Tìm hiểu thêm', 'eduma' ) . '</a>'; ?>
									</div>
									<div class="thim-course-content">
										...
										<?php if ( class_exists( 'LP_Addon_Coming_Soon_Courses_Preload' ) && learn_press_is_coming_soon( $course_item->ID ) ): ?>
											...
											<div
												...
												<?php esc_html_e( 'Sắp đăng tải', 'eduma' ) ?>
											</div>
										<?php else: ?>
											<div class="course-meta">
												...
												<div class="course-students">
													<label><?php esc_html_e( 'Học viên', 'eduma' ); ?></label>
												...
												<?php if ( $price = $course->get_price_html() ) {
													...
													if ( $course->is_free() || ! $is_required ) {
														$class .= ' free-course';
														$price = esc_html__( 'Miễn phí', 'eduma' );
													}
		...
		<aside class="thim-course-filter-wrapper">
			<form action="" name="thim-course-filter" method="POST" class="thim-course-filter">
				...
				if ( get_theme_mod( 'thim_filter_by_cate', false ) ) {
					...
					<h4 class="filter-title"><?php echo esc_html_x( 'Danh mục', 'Course filter widget', 'eduma' ); ?></h4>
				...
				if ( get_theme_mod( 'thim_filter_by_instructor', false ) ) {
					...
					?>
					<h4 class="filter-title"><?php echo esc_html_x( 'Giáo viên', 'Course filter widget', 'eduma' ); ?></h4>
					<?php
					if ( $course_query->have_posts() ) {
						global $post;
				...
				if ( get_theme_mod( 'thim_filter_by_price' ) ) {
					...
					?>
					<h4 class="filter-title"><?php echo esc_html_x( 'Giá', 'Course filter widget', 'eduma' ); ?></h4>
				...
				<div class="filter-submit">
					<button type="submit"><?php esc_html_e( 'Kết quả lọc', 'eduma' ) ?></button>
				</div>
			</form>
		</aside>
		<?php
	...
	function thim_courses_searching_callback() {
		...
		if ( $keyword ) {
			...
			if ( count( $search->posts ) ) {
				...
				$newdata[] = array(
					'id'    => '',
					'title' => '<i>' . esc_html__( 'Không có khóa học nào', 'eduma' ) . '</i>',
					'guid'  => '#',
				);
				echo json_encode( $newdata );
			}
...
if ( ! function_exists( 'show_pass_text' ) ) {
	function show_pass_text() {
		...
		if ( $grade == 'passed' ) {
			echo '<div class="message message-success learn-press-success">' . ( __( 'Xin chúc mừng bạn đã hoàn thành khóa học.', 'eduma' ) ) . '</div>';
		}
	}
}
					...
					<div class="container">
						<div class="thim-widget-heading thim-widget-heading-base">
							<div class="sc_heading clone_title  text-center">
								<h2 class="title"><?php esc_html_e( 'Khóa học phổ biến', 'eduma' ); ?></h2>
								<div class="clone"><?php esc_html_e( 'Khóa học phổ biến', 'eduma' ); ?></div>
							</div>
						</div>
								...
								<div class="course-item">
									...
									<div class="thim-course-content">
										...
										<div class="course-readmore">
											<a href="<?php echo esc_url( get_permalink() ); ?>"><?php esc_html_e( 'Tìm hiểu thêm', 'eduma' ); ?></a>
										</div>
									</div>
								</div>
							<?php
