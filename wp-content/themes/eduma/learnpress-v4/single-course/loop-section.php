...
<li<?php $section->main_class(); ?> id="section-<?php echo $section->get_slug(); ?>"
	...
	<h4 class="section-header">
		<div class="section-left">
			...
			echo ! $title ? _x( 'Không có tiêu đề', 'template title empty', 'eduma' ) : $title;
			?>
			<span class="meta">
	...
	<?php if ( ! $items ) : ?>
		<?php learn_press_display_message( __( 'Không có mục nào', 'eduma' ) ); ?>
	<?php else : ?>
					...
					<li class="<?php echo implode( ' ', $item->get_class() ); ?>"
						...
						<div class="meta-left">
							...
							<?php if ( $item->get_item_type() == 'lp_quiz' || $item->get_item_type() == 'lp_h5p' ) { ?>
								<div
									class="index"><?php echo '<span class="label">' . esc_html__( 'Bài tập', 'eduma' ) . '</span>' . $section->get_position() . '.' . $count[$post_type]; ?></div>
							<?php } elseif ( $item->get_item_type() == 'lp_assignment' ) { ?>
								<div
									class="index"><?php echo '<span class="label">' . esc_html__( 'Bài kiểm tra', 'eduma' ) . '</span>' . $section->get_position() . '.' . $count[$post_type]; ?></div>
							<?php } else { ?>
								<div
									class="index"><?php echo '<span class="label">' . esc_html__( 'Bài học', 'eduma' ) . '</span>' . $section->get_position() . '.' . $count[$post_type]; ?></div>
							<?php } ?>
						</div>
