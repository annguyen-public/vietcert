...
if ( $layout_type == 'grid' ) {
	...
	if ( $total == 0 ) {
		echo '<p class="message message-error">' . esc_html__( 'Không còn bài đăng nào nữa!', 'eduma' ) . '</p>';

		return;
	} elseif ( $total == 1 ) {
		$index = esc_html__( 'Chỉ có một bài đăng', 'eduma' );
	} else {
		...
		if ( $from == $to ) {
			$index = sprintf(
				esc_html__( 'Hiển thị bài đăng cuối cùng của %s kết quả', 'eduma' ),
				$total
			);
		} else {
			$index = sprintf(
